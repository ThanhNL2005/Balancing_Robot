...
{
	/*Motor parameters*/
	pMParas->f_Ia_rated = 4.5f;											//Ampere
	pMParas->f_Va_rated = 24.0f;										//V
	pMParas->f_Motor_Spd_rated = 14000.0f;						//rpm
	pMParas->f_Motor_Torque_rated = 12.34567/f_Gear_Ratio; 		//Nm
...
//			stMControl_Var.f_Va_SP = stMControl_Var.i_Dir*SP_Var.f_SI_AN_VaSP;							//Vol
		...
		}case REMOTE_CAN:
		{
			...
			if(stMControl_Var.f_Ia_SP >= 8.5f)
				stMControl_Var.f_Ia_SP = 8.5f;
			else if(stMControl_Var.f_Ia_SP <= -8.5f)
				stMControl_Var.f_Ia_SP = -8.5f;
...
//			stMControl_Var.f_Va_control = -(stPI_Ia.Usk - stMControl_Var.f_BackCalGain*stMControl_Var.f_Speed_SatErr);
	...
	if(ui16_50msEvent == 1){
		ui16_50msEvent = 0;
			vRPM_Cal(&stQEI_Var, &stMachine_Var);
			N_Round=N_Round+(float)(stQEI_Var.f_Cnt_CAN/(500.0f*4*f_Gear_Ratio));
