...
float f_Ti_Spd = 0.2; 						//second
float f_Ti_Ia = 0.01;							//second
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	vLPF_Init(&st_LPF_dotIa, 150, f_base_Ts);						//For dia/dt filter		
	vLPF_Init(&st_LPF_IaSP, 50, f_base_Ts);							//For Analog IaSP filter
	vLPF_Init(&st_LPF_Ia, 150, f_base_Ts);							//For Ia filter	
...
//	HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, GPIO_PIN_SET);		//Turn Off Led 2
	...
	vQEI_Config(&stQEI_Var,13, 4, f_Gear_Ratio, f_Spd_Ts);	
