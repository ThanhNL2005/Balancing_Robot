...
#define RX_SIZE                                                                \
  33 /* config legth data_frame*/ /* correct with frequence 200hz - Ts = 5     \
                                     ms*/
...
void process_frame(HWT906 *hwt906_data, uint8_t *data) {
  ...
  for (int i = 0; i < RX_SIZE / 11; i++) {
    ...
    if (frame[0] != 0x55 || sum != frame[10]) {
      ...
      int16_t data1 =
          COMBINE_2BYTES(frame[3], frame[2]); // Dữ liệu theo tọa độ x
      int16_t data2 =
          COMBINE_2BYTES(frame[5], frame[4]); // Dữ liệu theo tọa độ y
      int16_t data3 =
          COMBINE_2BYTES(frame[7], frame[6]); // Dữ liệu theo tọa độ z
