...
//			if(fabs(HWT906_data.angle.x)<0.3)hsmc->x4=0;
        smc->x5 = HWT901_data.angle.z * pi / 180.0f ;
        smc->x6 = HWT901_data.angular_velocity.z * pi / 180.0f;
...
//        fRampGen(&fRamp_x,hsmc->x1,0.00001f);
        ...
        smc->s3 = smc->C3 * smc->e5 + smc->e6;
				...
				smc->u_theta=(1/g->g2)*(-smc->c2*smc->e4 + x3d_dd-smc->ETA2*((smc->s1 > 0) - (smc->s2 < 0)) - smc->k2*smc->s2);
				smc->u_psi=(1/g->g3)*(-smc->C3*smc->e6 + x5d_dd-smc->ETA3*((smc->s3 > 0) - (smc->s3 < 0)) - smc->k3*smc->s3);	

//				smc->u_psi=PID_control(&pid_psi,angle_psi_1,smc->x5);
